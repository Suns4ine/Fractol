...
t_image		*mlx_win_init(void *mlx) //инициализирует изображение
{
	t_image	*image; //создаем картинку

	if (!(image = (t_image *)ft_memalloc(sizeof(t_image))))
		exit(0);
	image->image = mlx_new_image(mlx, WIDHT, HIGHT); //объявляем парматры
	image->data_addr = mlx_get_data_addr(image->image, &image->bpp, &image->sl, &image->endian); //узнать что она делает!!!!!!!!!!!!!!!!
	return (image);
}
...
void		choose_fract(t_fractol *data)
{
	...
	data->img = mlx_win_init(data->mlx); // инициализация
	pth_fun(data); // многопоточность
	mlx_hook(data->win, 4, 0, zoom, data); //работа мышки с функцией zoom
